...
def goal_act_fn(
    ...
    def act_fn(
        params: FrozenDict, observation: chex.Array, goal: chex.Array, key: chex.PRNGKey
    ) -> chex.Array:
        """Get the action from the distribution."""
        if rngs is None:
            pi = actor_apply(params, observation, goal)
        else:
            pi = actor_apply(params, observation, goal, rngs=rngs)
...
def obstacle_act_fn(
    ...
    def act_fn(
        ...
        logits = actor_apply(params, observation, goal).preferences - penalty
...
def get_goal_evaluator_fn(
    ...
    def eval_one_episode(params: FrozenDict, init_eval_state: EvalState) -> Dict:
        ...
        def _goal_env_step(params: FrozenDict, eval_state: EvalState) -> EvalState:
            """Step the environment."""
            # PRNG keys.
            key, env_state, last_timestep, step_count, episode_return = eval_state
        ...
        final_state = jax.lax.while_loop(not_done, _goal_env_step, init_eval_state)
...
def evaluator_setup(
    ...
    get_evaluator_fn = get_obst_evaluator_fn if obstacle else get_goal_evaluator_fn
